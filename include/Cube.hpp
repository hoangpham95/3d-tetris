...
class Cube {
 public:
  ...
  friend void ExportLocationAsCSV(std::ofstream& file, Cube** cubes,
                                  unsigned long size);
 ...
 private:
  ...
  // push all the neighboring Cube into a given queue, if the Cube is not in the
  // visited set
  friend void storeUnvisitedCube(const Cube* cube,
                                 std::unordered_set<Cube*>& store,
