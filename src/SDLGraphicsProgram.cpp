...
void SDLGraphicsProgram::GenerateBuffers() {
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, m_stride * sizeof(GLfloat), 0);
    glEnableVertexAttribArray(1);  // color
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, m_stride * sizeof(GLfloat),
                          (void*)(3 * sizeof(GLfloat)));
    glEnableVertexAttribArray(2);  // position
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, m_stride * sizeof(GLfloat), (void*)(7 * sizeof(GLfloat)));
    // index buffer
...
void SDLGraphicsProgram::makeCubes(int numCubes,
    ...
    GLint cubeHalfsizeUniformLocation = glGetUniformLocation(shader, "cubeHalfsize");
...
void SDLGraphicsProgram::loop() {
    ...
    while (!quit) {
        ...
        model = glm::translate(model, glm::vec3(-(float)tetris.GetX()/2.0f, 0.0f, -(float)tetris.GetZ()/2.0f));
                ...
                case SDL_KEYDOWN:
                    switch(e.key.keysym.sym) {
                        ...
                        case SDLK_q:  // pressed q
                            ...
                            break;
                        case SDLK_LEFT:
                            m_Direction = (rotate < PI/4 && rotate > -PI/4) ? D_LEFT : D_RIGHT;
                            break;
                        case SDLK_RIGHT:
                            m_Direction = (rotate < PI/4 && rotate > -PI/4) ? D_RIGHT : D_LEFT;
                            break;
                        case SDLK_r:
                            m_Rotation = R_CW;
                            break;
